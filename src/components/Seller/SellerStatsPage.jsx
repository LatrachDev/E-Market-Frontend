...
const SellerTopProducts = ({ topProducts }) => {
  if (!topProducts || topProducts.length === 0)
    return <p>No top products yet.</p>
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {topProducts.map((p) => (
        <div
          key={p.productId}
          className="rounded-xl border border-brandRed/10 bg-white p-4 shadow-sm flex flex-col items-center"
        >
          <img
            src={
              p.primaryImage
                ? `${import.meta.env.VITE_BASE_URL}${p.primaryImage}`
                : '/placeholder.png'
            }
            alt={p.title}
          ...
          <p className="text-sm text-gray-600">Sold: {p.quantitySold}</p>{' '}
...
const SellerStatsPage = () => {
  ...
  return (
    <div className="mx-auto max-w-7xl px-6 py-10 space-y-10">
      {' '}
      <h1 className="text-3xl font-semibold font-playfair text-gray-900">
