...
export default function Sidebar({ navLinks = [], activeSection, onSelect }) {
  ...
  return (
    <aside className="relative w-full bg-white shadow-sm lg:sticky lg:top-0 lg:h-screen lg:max-h-screen lg:w-72 lg:overflow-y-auto">
      <div
        className="absolute inset-x-0 top-0 h-24 bg-linear-to-br from-brandRed via-[#c35a4c] to-[#f0d6d1] opacity-90"
        aria-hidden="true"
      />
                ...
                <li key={link.id}>
                  ...
                  >
                    <span className="flex items-center gap-3">
                      <span
                        className={`flex h-8 w-8 items-center justify-center rounded-lg ${isActive ? 'bg-white/20 text-white' : 'bg-brandRed/10 text-brandRed'}`}
                      >
                        {link.icon}
                      </span>
                      {link.label}
      ...
      <div className="relative hidden border-t border-white/10 mt-5 px-6 py-6 lg:block">
        <button
          ...
          className="mt-4 w-full cursor-pointer rounded-lg bg-brandRed px-4 py-2 text-sm font-semibold font-montserrat text-white transition hover:bg-hoverBrandRed"
        >
          Logout
        </button>
      </div>
