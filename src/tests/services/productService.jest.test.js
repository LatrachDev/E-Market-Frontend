...
describe('ProductService - Tests avec Jest', () => {
  ...
  describe('fetchProducts', () => {
    ...
    it('devrait gérer response.data.products (format alternatif)', async () => {
      // Arrange
      const mockProducts = [{ _id: '1', title: 'Product 1', price: 19.99 }]
  ...
  describe('createProduct', () => {
    ...
    it('devrait throw une erreur si non vendeur (403)', async () => {
      ...
      const mockError = {
        response: {
          status: 403,
          data: { message: 'Only sellers can create products' },
        },
      }
      api.post.mockRejectedValue(mockError)
  ...
  describe('updateProduct', () => {
    ...
    it('devrait throw une erreur si pas le propriétaire (403)', async () => {
      ...
      // Act & Assert
      await expect(updateProduct(productId, formData)).rejects.toEqual(
        mockError
      )
      expect(consoleErrorSpy).toHaveBeenCalled()
    })
