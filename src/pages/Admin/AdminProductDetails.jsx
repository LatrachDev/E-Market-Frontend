...
const PLACEHOLDER =
  'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="400" height="400"><rect width="100%" height="100%" fill="%23f3f4f6"/><text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" font-family="Arial" font-size="20" fill="%239ca3af">No Image</text></svg>'
...
export default function AdminProductDetails() {
  ...
  useEffect(() => {
    if (!product) return
    const sellerId =
      product.seller_id || product.sellerId || product.seller?._id || null
    if (!sellerId) return
    let mounted = true
    ;(async () => {
      try {
  ...
  const gallery = [
    ...
    ...(Array.isArray(product.secondaryImages) ? product.secondaryImages : []),
  ].filter(Boolean)
  const img = gallery.length
    ? gallery[0].startsWith('http')
      ? gallery[0]
      : import.meta.env.VITE_API_BASE_URL
        ? import.meta.env.VITE_API_BASE_URL + gallery[0]
        : gallery[0]
    : PLACEHOLDER
        ...
        <div className="col-span-1">
          ...
          <div className="mt-4 flex gap-2">
            <button
              ...
              className="flex items-center w-full gap-2 px-4 py-2 bg-red-600 text-white rounded-md"
            >
              <Trash2 size={16} />
              Delete
            </button>
